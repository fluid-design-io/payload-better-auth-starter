...
export async function clearTestDb(payload: PayloadLike): Promise<void> {
	...
	if (!dbUri || !/_test\b/.test(dbUri)) {
		throw new Error('clearTestDb can only be used with a _test database (check DATABASE_URI)')
	}
	...
	const result = await pool.query(`SELECT tablename FROM pg_tables WHERE schemaname = $1`, [
		'public',
	])

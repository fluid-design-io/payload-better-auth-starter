			...
			{blogPosts.docs.map((article, index) => {
				...
				return (
					<div key={`${blogPost.title}-${blogPost.publishedAt}-${index}`}>
						<div
							aria-hidden
							className="h-px [background-image:linear-gradient(90deg,var(--color-foreground)_1px,transparent_1px)] bg-[length:4px_1px] bg-repeat-x opacity-20"
						/>
							...
							<div className="flex gap-4 max-md:justify-between md:grid md:w-40 md:grid-cols-[1fr_auto]">
								<div className="flex justify-end gap-2">
									{blogPost.authors?.map((author, authorIndex) => (
										<Avatar
											key={authorIndex}
											className="ring-border bg-card aspect-square size-6 overflow-hidden rounded-md border border-transparent shadow-md ring-1 shadow-black/15"
										>
											<AvatarImage
	...
	return (
		<div className="mx-auto flex flex-col gap-4">
			<Skeleton className="h-16 w-full rounded-md">
				<div className="h-px [background-image:linear-gradient(90deg,var(--color-foreground)_1px,transparent_1px)] bg-[length:4px_1px] bg-repeat-x opacity-20" />
			</Skeleton>
			<Skeleton className="h-16 w-full rounded-md">
				<div className="h-px [background-image:linear-gradient(90deg,var(--color-foreground)_1px,transparent_1px)] bg-[length:4px_1px] bg-repeat-x opacity-20" />
			</Skeleton>
